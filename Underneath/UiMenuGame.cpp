...
namespace Ui {
    ...
    void MenuGame::handleInput(int in){
        switch (in) {
            ...
            case 'x':
                currentLevel->newEntity(new AiEntity(AiType::aiMoveRandom, "A", new Geometry::Point2(cursorPosition->x+viewPos->x, cursorPosition->y+viewPos->y), Ui::C_DARK_RED));
                break;

            case 'c':
                currentLevel->newEntity(new AiEntity(AiType::aiMoveRandom, "B", new Geometry::Point2(cursorPosition->x+viewPos->x, cursorPosition->y+viewPos->y), Ui::C_LIGHT_GREEN));
                break;
